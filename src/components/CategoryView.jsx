            ...
            <div className="max-w-7xl mx-auto opacity-100 transition-opacity duration-500">
                <div className={`mb-10 p-8 md:p-12 rounded-[2.5rem] ${catData.theme.bg} flex flex-col md:flex-row items-center md:items-start gap-8 relative overflow-hidden shadow-sm border border-white dark:border-slate-700`}>
                    ...
                    <div className={`text-center relative z-10 ${isRtl ? 'md:text-right' : 'md:text-left'}`}>
                        ...
                        <p className="text-slate-700 dark:text-slate-300 text-lg md:text-xl max-w-3xl leading-relaxed font-medium">{localeCat.description}</p>
                    </div>
                </div>
                    ...
                    {catData.products.map((product) => {
                        ...
                        return (
                            <div
                                ...
                                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigateTo(catData, product); } }}
                                className="bg-white dark:bg-slate-900 rounded-[2rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl hover:border-slate-300 dark:hover:border-slate-700 transition-all duration-300 cursor-pointer group flex flex-col h-full focus-visible:ring-2 focus-visible:ring-blue-500/60 focus-visible:ring-offset-2 focus-visible:outline-none"
                            >
                                ...
                                <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed line-clamp-3 flex-grow font-medium">{localeProd.shortDesc}</p>
                                {PRICING[product.id] && (
