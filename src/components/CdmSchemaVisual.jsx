...
function EntityChip({ label, highlight = false }) {
    return (
        <div
            ...
            className={`text-[9px] sm:text-[10px] font-semibold rounded px-1.5 py-1 leading-tight text-center border break-words hyphens-auto min-w-0 ${highlight ? 'bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 border-slate-900 dark:border-slate-100 ring-2 ring-slate-700 dark:ring-slate-400 shadow' : 'bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 border-slate-300 dark:border-slate-700'}`}
        >
            {label}
        </div>
    );
...
function GroupPanel({ title, entities, dashed = false, accent = 'blue', highlightFirst = false, showDots = true }) {
    const accentMap = {
        blue: dashed
            ? 'bg-white/40 dark:bg-slate-900/40 border-slate-500 dark:border-slate-600'
            : 'bg-slate-200 dark:bg-slate-800 border-slate-400 dark:border-slate-700',
        teal: 'bg-slate-300 dark:bg-slate-700 border-slate-400 dark:border-slate-600',
        plain: 'bg-slate-100 dark:bg-slate-800/60 border-slate-300 dark:border-slate-700'
    };
    return (
        <div className={`rounded-md p-2 ${accentMap[accent]} ${dashed ? 'border border-dashed' : 'border'}`}>
            {title && (
                <p className="text-[11px] sm:text-xs font-extrabold tracking-wider uppercase mb-2 text-center text-slate-800 dark:text-slate-100">
                    {title}
                </p>
            )}
            <div className="space-y-1.5">
                ...
                {showDots && (
                    <div className="text-slate-600 dark:text-slate-300 text-center text-[10px] tracking-widest">•••</div>
                )}
            </div>
        </div>
    );
...
export default function CdmSchemaVisual({ lang }) {
    return (
        <section className="space-y-4">
            <div className="rounded-2xl bg-gradient-to-br from-slate-100 to-slate-300 dark:from-slate-900 dark:to-slate-800 p-3 sm:p-5 border border-slate-300 dark:border-slate-700 shadow-sm">
                {/* Header bar */}
                <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
                    <span className="bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 px-4 py-2 rounded text-xs sm:text-sm font-extrabold tracking-wider">
                        COMMON DATA MODEL SCHEMA
                    </span>
                    <span className="text-slate-800 dark:text-slate-100 font-extrabold text-sm sm:text-base tracking-wide">Dynamics 365</span>
                </div>
                ...
                <div className="relative grid gap-3 grid-cols-1 lg:grid-cols-12">
                    ...
                    <div className="lg:col-span-2">
                        <p className="text-[11px] sm:text-xs font-extrabold tracking-wider uppercase mb-2 text-slate-800 dark:text-slate-100 text-center">Core</p>
                        <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-1 gap-1.5">
                            {CORE_ENTITIES.map((e, i) => <EntityChip key={i} label={e} />)}
                            <div className="text-slate-600 dark:text-slate-300 text-center text-[10px] tracking-widest col-span-full lg:col-span-1">•••</div>
                        </div>
                        <p className="text-[11px] text-slate-700 dark:text-slate-300 mt-3 leading-snug text-center italic hidden lg:block">
                    ...
                    <div className="lg:col-span-10 space-y-3">
                        ...
                        <div className="bg-white/50 dark:bg-slate-900/50 rounded-lg p-2 border border-slate-300 dark:border-slate-700">
                        ...
                        <div className="grid gap-3 grid-cols-1 lg:grid-cols-12">
                            ...
                            <div id="cdm-account-card" className="lg:col-span-4 relative">
                                <div className="bg-slate-800 dark:bg-slate-950 border-2 border-slate-900 dark:border-slate-700 rounded-lg p-4 text-slate-100 shadow-md h-full">
                    ...
                    >
                        <defs>
                            <marker id="cdm-arrow-head" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
                                <path d="M0,0 L10,5 L0,10 z" className="fill-slate-900 dark:fill-slate-200" />
                            </marker>
                        </defs>
                ...
                <p className="text-[11px] text-slate-700 dark:text-slate-300 mt-4 leading-snug italic lg:hidden">
                    {getUI('cdmCoreNote', lang)}
                </p>
            </div>

            <p className="text-xs text-slate-500 dark:text-slate-400 italic">{getUI('cdmCaption', lang)}</p>
        </section>
    );
}
